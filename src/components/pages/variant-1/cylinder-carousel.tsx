...
export function CylinderCarousel() {
  document.title = 'El-Sombrart | Illustrator';
  ...
  useEffect(() => {
    ...
    const getResponsiveDimensions = () => {
      ...
      const maxRadius = isMobile ? 1.8 : isTablet ? 2.15 : 2.3;
      const cylinderHeight = isMobile ? 0.8 : isTablet ? 0.95 : 1.05;

      // Adjust camera distance based on screen size
      const cameraZ = isMobile ? 6 : isTablet ? 7.2 : 8.4;
    ...
    <>
      ...
      <div className="fixed inset-0 pointer-events-none z-10 text-white">
        {perspectives.map((perspective, index) => (
          ...
          >
            <h2 className="text-4xl sm:text-6xl lg:text-7xl font-[300] leading-[0.8]">{perspective.title}</h2>
            <p className="text-base sm:text-xl lg:text-2xl font-[300] opacity-50 mt-2">{perspective.description}</p>
          </div>
        ))}
      </div>

      <div className="fixed bottom-6 right-4 sm:right-6 lg:right-8 z-10 pointer-events-none">
        <div className="flex flex-col items-center gap-2 animate-bounce">
          ...
          <span className="text-xs sm:text-sm text-white/40">Scroll</span>
        </div>
      </div>
